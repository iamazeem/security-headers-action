...
analyze() {
  ...
  echo "Checking response for status"
  ...
  if [[ -n $INPUT_EXPECTED_GRADE ]]; then
    echo "Checking response for expected grade [$INPUT_EXPECTED_GRADE]"
    local GRADE="$(jq -rc '.summary.grade | ascii_upcase' <<<"$API_RESPONSE")"
